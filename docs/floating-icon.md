...
4.  **Persistence:** The position of the icon is saved automatically. If you disable and re-enable the feature, or reboot your device, the icon will reappear at the last saved location.

## Setup & Configuration

- **Enable/Disable:** Use the toggle on the home screen.
- **Permissions:** 
    - **Display over other apps:** Necessary to show the icon on top of other apps.
    - **Notifications:** Required to keep the service running in the background reliably.

## Technical Details

- **Service:** `FloatingIconService` handles the `WindowManager` logic and touch events.
